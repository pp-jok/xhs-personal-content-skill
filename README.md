...
- 可调教偏好：用户可以持续反馈“太 AI”“这个标题可以”等判断；只有明确确认的长期偏好才会写入正式规则。
- 用户态引导：默认用账号运营助手的方式交流，少暴露文件、命令、JSON 和内部流程。
- 来源和决策清晰：区分已有资料、客观数据、Codex 判断、候选规则和用户已决定事项。
...
- `RuleCard`：规则卡片。
- `RuleEvidence`：规则证据。
- `ProvenanceRecord`：来源追踪记录。
- `DecisionRequest`：用户决策记录。
- `ObjectVersion`：对象版本快照。
- `TopicItem`：选题。
- `ContentDraft`：内容草稿。
- `ContentQualityReview`：内容质量评价。
- `PublishTask`：发布任务。
- `OwnPost`：已发布帖子。
- `ReviewRecord`：复盘记录。
...
- 最小来源追踪与用户决策：记录来源、产生者和内容性质，候选规则需要用户确认后才长期生效。
- 重要对象版本快照：账号档案、规则卡片和内容草稿更新时保留上一版本。
- 采集内容拆解：把可见内容拆成事实、推断、不确定项、账号适配判断和候选规则引用。
- 提升为对标：运营人员确认后，可把采集内容提升为对标账号和对标帖子草稿。
- 规则生命周期：候选、确认、测试、验证、拒绝、废弃，并保留证据和验证次数。
- 质量提升验证：记录草稿质量评价，生成每周质量报告，观察修改成本是否下降。
...
- 5 到 20 个自定义标签或偏好反馈。
- 3 到 10 条草稿质量评价，用于观察规则是否真的提高内容可用度。
