...
第一版目标：显式选择 1 个 active ContentAsset 作为生成参考进入 GenerationContext、TopicItem 和 ContentDraft 的审计快照。

当前边界：

```text
ContentMechanism 不进入 GenerationContext。
ContentAsset 只允许 active 状态被显式引用。
ContentAsset 必须属于本次生成使用的账号档案。
candidate / deprecated ContentAsset 不允许进入生成。
一次生成最多引用 1 个 ContentAsset。
如果 TopicItem 已保存资产引用，Draft 生成不得静默替换为另一个显式资产。
不自动读取全部 active assets。
不做资产推荐、排序或筛选。
不执行 {{variable_name}} 模板替换。
不修改 ContentAsset 生命周期状态。
```
...
`--asset-version` 可选，用于拒绝过期资产引用；不提供时使用当前 active 资产版本形成快照。

`--reference-mechanism-id` 继续暂缓，避免把软知识直接带入正式生成。
